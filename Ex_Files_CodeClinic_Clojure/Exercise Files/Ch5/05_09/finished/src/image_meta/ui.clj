...
(defn on-submit
  [frame]
  (let [{:keys [src target copy]} @ui-data]
    (when (and src target)
      (backend/arrange-images copy src target))))

(defn add-listeners
  [frame]
  (let [{:keys [src target copy submit]} (ss/group-by-id frame)]
    (ss/listen src :mouse-clicked (fn [_] (choose-dir frame :src)))
    (ss/listen target :mouse-clicked (fn [_] (choose-dir frame :target)))
    (ss/listen copy :mouse-clicked #(swap! ui-data assoc :copy (ss/value %)))
